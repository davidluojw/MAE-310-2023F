...
int main()
{
    ...
    SparseMatrixType K(n_eq, n_eq);            // Create a n_eqxn_eq sparse matrix
    Eigen::VectorXd F(n_eq);                     //Right-hand side vector
        ...
        for (int aa = 0; aa < n_en; ++aa)
        {
            ...
            if (PP > 0)
            {
                ...
                for (int bb = 0; bb < n_en; ++bb)
                {
                    ...
                    if (QQ > 0)
                    {
                        K.coeffRef(PP - 1, QQ - 1) = K.coeffRef(PP - 1, QQ - 1)+ k_ele[aa * n_en + bb];
                    } 
                    else 
                    {
    ...
    K.makeCompressed();     //Compress the sparse matrix for efficient storage and computation
    ...
    Eigen::SparseLU<SparseMatrixType> sparseLuDecomp;
    sparseLuDecomp.analyzePattern(K);
    sparseLuDecomp.factorize(K);
    Eigen::VectorXd d_temp = sparseLuDecomp.solve(F);        // Solve the linear system
